...
public class PatrolPath : MonoBehaviour
{
   ...
   public PathPoint GetClosestPathPoint(Vector2 tankPosition)
   {
      ...
      return new PathPoint {indexStruct = index, positionStruct = patrolPoints[index].position};
   }

   public PathPoint GetNextPathPoint(int index)
   {
      var newIndex = index + 1 >= patrolPoints.Count ? 0 : index + 1;
      return new PathPoint { indexStruct = newIndex, positionStruct = patrolPoints[newIndex].position };
   }
